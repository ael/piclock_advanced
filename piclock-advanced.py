#! /usr/bin/env python
# -*- coding: utf-8 -*-
import pygame, sys, math, time, os, codecs, socket, json
...
useudpbroadcast = True  # set to true if you want to use udp broadcast to update song and studio info
UDP_PORT = 5005
mictimer_time = 0
...
while True:
    ...
    # listen on UDP-Port and process incoming messages
    if useudpbroadcast == True:
        if firstrun == True:
            udpsonginfo = str(readsonginfo())
            udpstudioinfo = str(readonairstudio())
            firstrun = False
        ...
        try:
            if "studio" in message:
                udpstudioinfo = message["studio"]["value"]
            if "song" in message:
                udpsonginfo = message["song"]["value"]
            else:
                print("Fehler beim UDP Lesen!")
        except:
            pass
    ...
    if useudpbroadcast == True:
        # Update Songinfo UDP
        songinfo = infofont.render(str(udpsonginfo), True, txtcolour)
    else:
        # Update songinfo SMB
